...
// OpenGL version
const unsigned int GL_VERSION_MAJOR = 2;
const unsigned int GL_VERSION_MINOR = 0;
...
unsigned int displayWidth = 960;
unsigned int displayHeight = 544;
...
void LoadModel()
{
	...
	else
	{
		...
		for (unsigned int i = 0; i < scene->mNumMeshes; i++)
		{
			...
			glGenVertexArraysOES(1, &mesh->vertexArrayObject);
							  
			// Tell OpenGL how to interpret the model data
			glBindVertexArrayOES(mesh->vertexArrayObject);
...
void UnloadModel()
{
	...
	for(Mesh* mesh : model->meshes)
	{
		...
		// Delete vertex array
		glDeleteVertexArraysOES(1, &mesh->vertexArrayObject);
...
void Update(float deltaTime)
{
    ...
    int z = SDL_GameControllerGetAxis(gamecontroller, SDL_CONTROLLER_AXIS_RIGHTX);
    int zz = SDL_GameControllerGetAxis(gamecontroller, SDL_CONTROLLER_AXIS_RIGHTY);
    ...
    if (y < -15000)
		modelRotation += glm::vec3(Radians(-45.0f),0.0f, 0.0f) * deltaTime;
    if (y > 15000)
		modelRotation += glm::vec3(Radians(45.0f), 0.0f, 0.0f) * deltaTime;
